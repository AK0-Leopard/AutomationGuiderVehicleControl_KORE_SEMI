...
#pragma warning restore CS0414 // 已指派欄位 'DebugForm.blocked_queues'，但從未使用過其值。
        ...
        public DebugForm(BCMainForm _mainForm)
        {
            ...
            ck_ActiveHPRScenario.Checked = DebugParameter.isActiveHPRScenario;
            ...
            BCUtility.setComboboxDataSource(cmbHPRVehicle, allVh);
        ...
        private void btn_ReserveTest_Click(object sender, EventArgs e)
        {
            ...
            ReserveEventTest(reserves);
            //bcApp.SCApplication.ReserveBLL.IsReserveSuccessTest(noticeCar.VEHICLE_ID, reserves);

        }
        private void ReserveEventTest(Google.Protobuf.Collections.RepeatedField<sc.ProtocolFormat.OHTMessage.ReserveInfo> reserves)
        {
            string cst_id = txt_mcsReportTestCstID.Text;
            string cmd_id = txt_mcsReportTestCmdID.Text;
            AVEHICLE test_report_vh = bcApp.SCApplication.VehicleBLL.cache.getVehicle(cmb_tcpipctr_Vehicle.Text);
            var id_136 = new sc.ProtocolFormat.OHTMessage.ID_136_TRANS_EVENT_REP()
            {
                EventType = sc.ProtocolFormat.OHTMessage.EventType.ReserveReq,
                CSTID = cst_id,
                BCRReadResult = sc.ProtocolFormat.OHTMessage.BCRReadResult.BcrNormal,
                CmdID = cmd_id,
            };
            id_136.ReserveInfos.AddRange(reserves);
        ...
        private async void btn_changeToRemove_Click(object sender, EventArgs e)
        {
            try
            {
                if (!noticeCar.IS_INSTALLED)
                {
                    ...
                    //有可能車輛在沒有INSTALL的情況下上線又離線了，所以在已Remove的情況下，還是要做移除車輛的動作
                    await Task.Run(() => bcApp.SCApplication.VehicleService.Remove(vh_id, false));
                    return;
                }
                btn_changeToRemove.Enabled = false;
                (bool isSuccess, string result) check_result = default((bool isSuccess, string result));
                await Task.Run(() => check_result = bcApp.SCApplication.VehicleService.Remove(vh_id,true));
        ...
        private void btn_emptyFoupRequest_Click(object sender, EventArgs e)
        {
            string Port_id = txt_mcsReportTestPortID.Text.Trim();
            Task.Run(() => bcApp.SCApplication.ReportBLL.ReportEmptyFoupRequest(Port_id));
        }

        private void btn_emptyFoupDepositFail_Click(object sender, EventArgs e)
        {
            string Port_id = txt_mcsReportTestPortID.Text.Trim();
            Task.Run(() => bcApp.SCApplication.ReportBLL.ReportEmptyFoupDepositFail(Port_id));
        }
        AVEHICLE HPRCar = null;
        string HPR_vh_id = null;
        private void cmbHPRVehicle_SelectedIndexChanged(object sender, EventArgs e)
        {
            HPR_vh_id = cmbHPRVehicle.Text.Trim();
            HPRCar = bcApp.SCApplication.getEQObjCacheManager().getVehicletByVHID(HPR_vh_id);
            if (HPRCar != null)
            {
                lb_VehicleHPRStatus.Text = HPRCar.isHPRVehicle.ToString();
            }
        }

        private void btn_SetHPRVehicle_Click(object sender, EventArgs e)
        {
            try
            {
                if (HPRCar.isHPRVehicle)
                {
                    MessageBox.Show($"{HPR_vh_id} is HPR Vehicle already!");
                    return;
                }

                btn_SetHPRVehicle.Enabled = false;
                HPRCar.isHPRVehicle = true;
                lb_VehicleHPRStatus.Text = HPRCar.isHPRVehicle.ToString();
            }
            finally
            {
                btn_SetHPRVehicle.Enabled = true;
            }
        }

        private void btn_RemoveHPRVehicle_Click(object sender, EventArgs e)
        {
            try
            {
                if (!HPRCar.isHPRVehicle)
                {
                    MessageBox.Show($"{HPR_vh_id} is not HPR Vehicle already!");
                    return;
                }

                btn_RemoveHPRVehicle.Enabled = false;
                HPRCar.isHPRVehicle = false;
                lb_VehicleHPRStatus.Text = HPRCar.isHPRVehicle.ToString();
            }
            finally
            {
                btn_RemoveHPRVehicle.Enabled = true;
            }
        }

        private void ck_ActiveHPRScenario_CheckedChanged(object sender, EventArgs e)
        {
