...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class VehicleBLL
    {
        ...
        public bool hasVehicleOnAddress(string address)
        {
            List<AVEHICLE> vhs = cache.loadAllVh();
            foreach (AVEHICLE vh in vhs)
            {
                if (!vh.isTcpIpConnect) continue;
                if (SCUtility.isMatche(vh.CUR_ADR_ID, address))
                {
                    return true;
                }
            }
            ...
            (string vhID, ID_134_TRANS_EVENT_REP report_obj)
        {
            ...
            var check_result = vh.CurrentGuideData.tryGetWalkDirOnSection(current_sec_id);
            ...
            //       Data: $"current_x_axis:{current_x_axis} or current_y_axis:{current_y_axis} is negative value,by pass this one.",
            ...
            //if (!update_result.OK)
            //{vehicles
            //    string message = $"The  bumped, vh:{vh.VEHICLE_ID} with vh:{update_result.VehicleID}";
            //    LogHelper.Log(logger: logger, LogLevel: LogLevel.Warn, Class: nameof(VehicleBLL), Device: Service.VehicleService.DEVICE_NAME_AGV,
            //       Data: message,
        ...
        public class Cache
        {
            ...
            public void setWillPassSectionInfo(sc.BLL.ReserveBLL reserveBLL, string vhID, GuideInfo guideInfo)
            {
                var vh = eqObjCacheManager.getVehicletByVHID(vhID);

                vh.CurrentGuideData.put(guideInfo.converToGuideData(reserveBLL));
                vh.PredictSections = guideInfo.GuideSections.ToArray();
                vh.WillPassSectionID = guideInfo.GuideSections.ToList();
                vh.sWillPassAddressIDs = string.Join(",", guideInfo.GuideAddresses);
                vh.ToAdr = guideInfo.GuideAddresses.Last();
                vh.ToSectionID = guideInfo.GuideSections.Last();
            ...
            public void removeAlreadyPassedSection(string vhID, string sectionID)
            {
                ...
                vh.CurrentGuideData?.refreshPassStatus(sectionID);
            }
            ...
            public AVEHICLE findBestSuitableVhStepByStepFromAdr(GuideBLL GuideBLL, CMDBLL cmdBLL, string source, E_VH_TYPE vh_type,
                bool isHPRCommand = false, bool hasEmptyCST = false)
            {
            ...
            public void filterCanNotExcuteTranHPRVh(ref List<AVEHICLE> vhs, E_VH_TYPE vh_type)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    ...
                    if (!vh.isHPRVehicle)
                    {
                        ...
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterCanNotExcuteTranHPRVh method, vh id:{vh.VEHICLE_ID} is not HPR vehicle, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
            ...
            public void filterHPRVh(ref List<AVEHICLE> vhs)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.isHPRVehicle)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterHPRVh method, vh id:{vh.VEHICLE_ID} is HPR vehicle, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
            ...
            public void filterNonHPRVh(ref List<AVEHICLE> vhs)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (!vh.isHPRVehicle)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterNonHPRVh method, vh id:{vh.VEHICLE_ID} is not HPR vehicle, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
            ...
            public void filterCommandingVh(ref List<AVEHICLE> vhs)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.ACT_STATUS != VHActionStatus.NoCommand)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterCommandingVh method, vh id:{vh.VEHICLE_ID} is excuting command, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
            ...
            public void filterNoEmptyCSTVh(ref List<AVEHICLE> vhs)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.HAS_CST_L && vh.HAS_CST_R)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} do not have empty slot for carrier, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
                ...
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (!vh.HAS_CST_L && !vh.HAS_CST_R)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} do not have empty carrier on vehicle, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
                ...
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.HAS_CST_L)
                    {
                        ACARRIER cst = SCApplication.getInstance().CarrierBLL.db.getCarrier(vh.CST_ID_L);
                        if (cst == null)
                        {
                            vhs.Remove(vh);
                            LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                               Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} can not find Carrier Data in Database, CarrierID:[{vh.CST_ID_L}], " +
                                     $"so filter it out",
                               VehicleID: vh.VEHICLE_ID,
                               CST_ID_L: vh.CST_ID_L,
                               CST_ID_R: vh.CST_ID_R);
                        }
                        else if (!SCUtility.isMatche(cst.HOSTDESTINATION, SCApplication.getInstance().HPREmptyCarrierVirtualPort))//不是空CST
                        {
                            vhs.Remove(vh);
                            LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                               Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} Carrier is not empty, CarrierID:[{vh.CST_ID_L}] Destination[{cst.HOSTDESTINATION}], " +
                                     $"so filter it out",
                               VehicleID: vh.VEHICLE_ID,
                               CST_ID_L: vh.CST_ID_L,
                               CST_ID_R: vh.CST_ID_R);
                    ...
                    if (vh.HAS_CST_R)
                    {
                        ACARRIER cst = SCApplication.getInstance().CarrierBLL.db.getCarrier(vh.CST_ID_R);
                        if (cst == null)
                        {
                            vhs.Remove(vh);
                            LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                               Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} can not find Carrier Data in Database, CarrierID:[{vh.CST_ID_R}], " +
                                     $"so filter it out",
                               VehicleID: vh.VEHICLE_ID,
                               CST_ID_L: vh.CST_ID_L,
                               CST_ID_R: vh.CST_ID_R);
                        ...
                        else if (!SCUtility.isMatche(cst.HOSTDESTINATION, SCApplication.getInstance().HPREmptyCarrierVirtualPort))//不是空CST
                        {
                            vhs.Remove(vh);
                            LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                               Data: $"In filterNoEmptyCSTVh method, vh id:{vh.VEHICLE_ID} Carrier is not empty, CarrierID:[{vh.CST_ID_R}] Destination[{cst.HOSTDESTINATION}], " +
                                     $"so filter it out",
                               VehicleID: vh.VEHICLE_ID,
                               CST_ID_L: vh.CST_ID_L,
                               CST_ID_R: vh.CST_ID_R);
            ...
            public void filterHasCSTVh(ref List<AVEHICLE> vhs)
            {
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.HAS_CST_L)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterhasCSTVh method, vh id:{vh.VEHICLE_ID} have cst on left slot, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
                ...
                foreach (AVEHICLE vh in vhs.ToList())
                {
                    if (vh.HAS_CST_R)
                    {
                        vhs.Remove(vh);
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleBLL), Device: "AGVC",
                           Data: $"In filterhasCSTVh method, vh id:{vh.VEHICLE_ID} have cst on right slot, " +
                                 $"so filter it out",
                           VehicleID: vh.VEHICLE_ID,
                           CST_ID_L: vh.CST_ID_L,
                           CST_ID_R: vh.CST_ID_R);
            ...
            {
                var vhs = eqObjCacheManager.getAllVehicle().
                          Where(vh => !vh.IsOnAdr && !SCUtility.isMatche(vh.VEHICLE_ID, byPassVhID)).
