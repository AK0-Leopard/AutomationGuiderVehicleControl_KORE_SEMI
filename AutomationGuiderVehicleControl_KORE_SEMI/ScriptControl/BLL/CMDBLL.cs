...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class CMDBLL
    {
        ...
        public bool HasCmdIsGoing(List<string> addresses)
        {
            ...
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                count = cmd_ohtcDAO.getExcuteCMDConutByAddresses(con, addresses);
            }
            return count > 0;
        }
        ...
        public bool hasExcuteCMDBySourcePortOrDestinationPort(string port)
        {
            int count = 0;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                count = cmd_ohtcDAO.getExcuteCMDConutLoadPortOrUnloadPort(con, port);
            }
            return count != 0;
        }
        ...
        public class Cache
        {
            ...
            public bool AddresshasCmdExcuted(string destAdr)
            {
                try
                {
                    ALINE line = app.getEQObjCacheManager().getLine();
                    ...
                    if (current_unfinish_cmd == null || current_unfinish_cmd.Count == 0)
                        return false;
                    var address_has_unfinish_cmd = current_unfinish_cmd.Where(cmd => SCUtility.isMatche(cmd.DESTINATION, destAdr))
                                                                         .Count() > 0;
                    return address_has_unfinish_cmd;
                }
                catch (Exception ex)
                {
                    logger.Error(ex, "Exception");
            ...
            public (bool hasExcuteCmd, List<ACMD> excuteCmds) tryGetExcuteCmds(string vhID)
            {
                try
                {
                    ALINE line = app.getEQObjCacheManager().getLine();
                    ...
                    if (current_unfinish_cmd == null || current_unfinish_cmd.Count == 0)
                        return (false, null);
                    var unfinish_cmd_by_vh = current_unfinish_cmd.Where(cmd => SCUtility.isMatche(cmd.VH_ID, vhID)).ToList();
                    return (unfinish_cmd_by_vh != null && unfinish_cmd_by_vh.Count > 0, unfinish_cmd_by_vh);
                }
                catch (Exception ex)
                {
                    logger.Error(ex, "Exception");
            ...
            public VTRANSFER getTransferCmd(string cmdID)
            {
                try
                {
                    ALINE line = app.getEQObjCacheManager().getLine();
                    var current_unfinish_transfer = line.CurrentExcuteTransferCommand;
                    if (current_unfinish_transfer == null || current_unfinish_transfer.Count == 0)
                        return null;
                    var has_unfinish_cmd_by_vh = current_unfinish_transfer.Where(cmd => SCUtility.isMatche(cmd.ID, cmdID)).
                                                                                FirstOrDefault();
                    return has_unfinish_cmd_by_vh;
                }
                catch (Exception ex)
