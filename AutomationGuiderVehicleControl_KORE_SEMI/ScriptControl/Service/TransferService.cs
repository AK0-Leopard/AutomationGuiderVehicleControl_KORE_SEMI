...
namespace com.mirle.ibg3k0.sc.Service
{
    public class TransferService
    {
        ...
        public bool Update(string transferID, int priority)
        {
            ...
            {
                is_success = scApp.CMDBLL.updateCMD_MCS_Priority(mcs_cmd, priority);
            ...
            if (System.Threading.Interlocked.Exchange(ref syncTranCmdPoint, 1) == 0)
            {
                try
                {
                        ...
                        (scApp.getEQObjCacheManager().getLine().SCStats == ALINE.TSCState.AUTO && scApp.getEQObjCacheManager().getLine().MCSCommandAutoAssign))
                    {
                        ...
                        scApp.LineService.ProcessAlarmReport("AGVC", AlarmBLL.AGVC_NO_AVALIBLE_HPR_VEHICLE, ProtocolFormat.OHTMessage.ErrorStatus.ErrSet,
                                        $"AGVC do not have avalible HPR vehicle.");
                ...
                try
                {
                        ...
                        (scApp.getEQObjCacheManager().getLine().SCStats == ALINE.TSCState.AUTO && scApp.getEQObjCacheManager().getLine().MCSCommandAutoAssign))
                    {
                        List<VTRANSFER> excuting_transfer = un_finish_trnasfer.
                                                    Where(tr => tr.TRANSFERSTATE > E_TRAN_STATUS.Queue &&
                                                                tr.TRANSFERSTATE <= E_TRAN_STATUS.Transferring &&
                                                                !tr.HOSTSOURCE.StartsWith("HPR") &&
                                                                !tr.HOSTSOURCE.StartsWith(scApp.HPREmptyCarrierVirtualPort) &&
                                                                !SCUtility.isMatche(tr.HOSTDESTINATION, scApp.HPREmptyCarrierVirtualPort) &&
                                                                !SCUtility.isEmpty(tr.VH_ID)).
                                                    ToList();
                        List<VTRANSFER> in_queue_transfer = un_finish_trnasfer.
                                                    Where(tr => tr.TRANSFERSTATE == E_TRAN_STATUS.Queue &&
                        ...
                        List<VTRANSFER> hpr_queue_transfer = un_finish_trnasfer.
                                                    Where(tr => tr.TRANSFERSTATE == E_TRAN_STATUS.Queue &&
                                                                (tr.HOSTSOURCE.StartsWith("HPR")
                                                                || tr.HOSTSOURCE.StartsWith(scApp.HPREmptyCarrierVirtualPort)
                                                                || SCUtility.isMatche(tr.HOSTDESTINATION, scApp.HPREmptyCarrierVirtualPort))).
                                                    ToList();//LoadPort是HPR EQ跟目的地是虛擬Port的要HPR專車來處理
                        ...
                        try
                        {
                            var traget_not_agv_st_hpr_queue_transfer = hpr_queue_transfer.Where(tran => !(tran.getTragetPortEQ(scApp.EqptBLL) is IAGVStationType))
                                                                 .OrderByDescending(tran => tran.PORT_PRIORITY)
                                                                 .ToList();
                            ...
                            if (traget_not_agv_st_hpr_queue_transfer != null&& traget_not_agv_st_hpr_queue_transfer.Count != 0)
                            {
                                ...
                                {
                                    DebugParameter.isActiveHPRScenario = true;
                                    scApp.LineService.ProcessAlarmReport("AGVC", AlarmBLL.AGVC_NO_AVALIBLE_HPR_VEHICLE, ProtocolFormat.OHTMessage.ErrorStatus.ErrReset,
                                    $"AGVC do not have avalible HPR vehicle.");
                                }
                                else//沒有專用車要報Alarm並改為跑一般車模式
                                {
                                    DebugParameter.isActiveHPRScenario = false;
                                    scApp.LineService.ProcessAlarmReport("AGVC", AlarmBLL.AGVC_NO_AVALIBLE_HPR_VEHICLE, ProtocolFormat.OHTMessage.ErrorStatus.ErrSet,
                                                $"AGVC do not have avalible HPR vehicle.");
                                    return;
                                }
                            ...
                            foreach (VTRANSFER first_waitting_excute_mcs_cmd in traget_not_agv_st_hpr_queue_transfer)
                            {
                                string hostsource = first_waitting_excute_mcs_cmd.HOSTSOURCE;
                                string hostdest = first_waitting_excute_mcs_cmd.HOSTDESTINATION;
                                string from_adr = string.Empty;
                                ...
                                AVEHICLE bestSuitableVh = null;
                                E_VH_TYPE vh_type = E_VH_TYPE.None;
                                bool isToLoadEmptyCST = false;
                                bool isToUnloadEmptyCST = false;
                                if (SCUtility.isMatche(hostdest, scApp.HPREmptyCarrierVirtualPort))//目的地是虛擬Port，這個是在空盒的命令
                                {
                                    isToLoadEmptyCST = true;
                                }
                                else if (hostsource.StartsWith(scApp.HPREmptyCarrierVirtualPort))
                                {
                                    isToLoadEmptyCST = false;
                                }
                                ...
                                if (source_is_a_port)
                                {
                                    ...
                                    if (isToLoadEmptyCST)
                                    {
                                        bestSuitableVh = scApp.VehicleBLL.cache.findBestSuitableVhStepByStepFromAdr(scApp.GuideBLL, scApp.CMDBLL, from_adr, vh_type, true, false);
                                    }
                                    else if (isToUnloadEmptyCST)
                                    {
                                        ACARRIER cARRIER = scApp.CarrierBLL.db.getCarrier(first_waitting_excute_mcs_cmd.CARRIER_ID);
                                        if (cARRIER != null)
                                        {
                                            bestSuitableVh = scApp.VehicleBLL.cache.getVehicleByCSTID(cARRIER.ID);
                                        }
                                    }
                                    else
                                    {
                                        bestSuitableVh = scApp.VehicleBLL.cache.findBestSuitableVhStepByStepFromAdr(scApp.GuideBLL, scApp.CMDBLL, from_adr, vh_type, true, true);
                                    }
                                }
                                else
                                {
                        ...
                        foreach (VTRANSFER queue_tran in hpr_queue_transfer)
                        {
                            //int AccumulateTime_minute = 5;
                            int AccumulateTime_minute = SystemParameter.TransferCommandTimePriorityIncrement;
                            int current_time_priority = ((int)((DateTime.Now - queue_tran.CMD_INSER_TIME).TotalMinutes) * AccumulateTime_minute);
    ...
    public class TranAssignerNormal : ITranAssigner
    {
        ...
        {
            bool is_success = true;
            ACMD assign_cmd = null;
            if (SCUtility.isMatche(waittingExcuteMcsCmd.HOSTDESTINATION, scApp.HPREmptyCarrierVirtualPort)) //目的是HPR空CST專用的虛擬Port，需要把命令改成單純Load命令給AGV。
            {
                assign_cmd = waittingExcuteMcsCmd.ConvertToCmdForLoadHPREmptyCST(scApp.PortStationBLL, scApp.SequenceBLL, bestSuitableVh);
            }
            else if (SCUtility.isMatche(waittingExcuteMcsCmd.HOSTSOURCE, scApp.HPREmptyCarrierVirtualPort))
            {
                assign_cmd = waittingExcuteMcsCmd.ConvertToCmdForUnloadHPREmptyCST(scApp.PortStationBLL, scApp.SequenceBLL, bestSuitableVh);
            }
            else
            {
                assign_cmd = waittingExcuteMcsCmd.ConvertToCmd(scApp.PortStationBLL, scApp.SequenceBLL, bestSuitableVh);
            }
