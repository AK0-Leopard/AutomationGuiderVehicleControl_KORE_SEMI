...
namespace com.mirle.ibg3k0.sc.Service
{
    ...
    public class VehicleService : IDynamicMetaObjectProvider
    {
        ...
        public class SendProcessor
        {
                                                         ...
                                                         string loadPort, string unloadPort)
            {
                ...
                else
                {
                    ...
                    //20210629 在AT&S發生time out還是要當作失敗來處理
            ...
            public (bool is_success, string result) Avoid(string vh_id, string avoidAddress)
            {
                ...
                try
                {
                    ...
                    if (is_success)
                    {
                        ...
                        if (!vh.IsOnAdr)
                        {
                            string start_section = SCUtility.Trim(vh.CUR_SEC_ID, true);
                            bool start_section_is_same = true;
                            if (guide_section_ids != null && guide_section_ids.Count > 0)
                            {
                                start_section_is_same = SCUtility.isMatche(guide_section_ids[0], start_section);
                                if (!start_section_is_same)
                                {
                                    guide_section_ids.Insert(0, start_section);
            ...
            private bool sendMessage_ID_51_AVOID_REQUEST(string vh_id, string avoidAddress, string[] guideSection, string[] guideAddresses)
            {
                ...
                if (isSuccess)
                {
                    ...
                    guideInfo.GuideSections.AddRange(guideSection);
                    guideInfo.GuideAddresses.AddRange(guideAddresses);
                    scApp.VehicleBLL.cache.setWillPassSectionInfo(scApp.ReserveBLL, vh.VEHICLE_ID, guideInfo);
                }
                return isSuccess;
            }
        ...
        public class ReceiveProcessor : IDynamicMetaObjectProvider
        {
            ...
            {
                Task.Run(() => scApp.VehicleBLL.setAndPublishPositionReportInfo2Redis(vh.VEHICLE_ID, recive_str));
                ...
                if (completeStatus != CompleteStatus.VehicleAbort)
                {
            ...
            //    if (!has_cmd_excute)
            //    {
            //        scApp.VehicleChargerModule.askVhToParkForWait(vh);
            //    }
            //}
            ...
            public void TranEventReport(BCFApplication bcfApp, AVEHICLE vh, ID_136_TRANS_EVENT_REP recive_str, int seq_num)
            {
                ...
                switch (eventType)
                {
                    ...
                    case EventType.Initial:
                        TransferReportInitial(bcfApp, vh, seq_num, eventType, carrier_id, cst_location);
                        break;
                    default:
            ...
            private void TransferReportInitial(BCFApplication bcfApp, AVEHICLE eqpt, int seq_num, EventType eventType, string cstID, AGVLocation cstLocation)
            {
                try
                {
                    ...
                    //CompleteStatus completeStatus = CompleteStatus.ForceNormalFinishByOp;
                    //if (excuteCmdsCheck.hasExcuteCmd)
                    //{
                    //    ACMD cmd = excuteCmdsCheck.excuteCmds[0];
                    //    if (cmd.IsTransferCmdByMCS)
                    //    {
                    //        LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //           Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, 開始結束流程... ",
                    //           VehicleID: eqpt.VEHICLE_ID,
                    //           CST_ID_L: eqpt.CST_ID_L);

                    //        VTRANSFER tran = cmdBLL.cache.getTransferCmd(cmd.TRANSFER_ID);
                    //        if (has_cst_on_vh)
                    //        {
                    //            LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //               Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, 有CST 在車上 CST ID:{cstID}",
                    //               VehicleID: eqpt.VEHICLE_ID,
                    //               CST_ID_L: eqpt.CST_ID_L);

                    //            if (is_bcr_read_fail)
                    //            {
                    //                final_cst_id = SCUtility.Trim(tran.CARRIER_ID, true);
                    //                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //                   Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial,initial時發生了 Read fail ID:{cstID}. 將改回命令的CST ID:{final_cst_id}",
                    //                   VehicleID: eqpt.VEHICLE_ID,
                    //                   CST_ID_L: eqpt.CST_ID_L);
                    //            }
                    //            else
                    //            {
                    //                is_mismatch = !SCUtility.isMatche(cstID, tran.CARRIER_ID);
                    //                final_cst_id = SCUtility.Trim(tran.CARRIER_ID, true);
                    //                if (is_mismatch)
                    //                {
                    //                    completeStatus = CompleteStatus.IdmisMatch;
                    //                    scApp.CarrierBLL.db.updateRenameID(tran.CARRIER_ID, cstID);
                    //                    LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //                       Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial,initial時發生了 ID mismatch:{cstID}. 命令 cst ID:{tran.CARRIER_ID},將其改成 cst id:{final_cst_id}",
                    //                       VehicleID: eqpt.VEHICLE_ID,
                    //                       CST_ID_L: eqpt.CST_ID_L);
                    //                }
                    //            }

                    //            var install_result = scApp.TransferService.tryInstallCarrierInVehicle(eqpt.VEHICLE_ID, eqpt.LocationRealID_L, final_cst_id);
                    //            LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //               Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial," +
                    //                     $"最後進行帳料調整 try install carrier:{final_cst_id} in vh,is success:{install_result.isSuccess} resutl:{install_result.result}",
                    //               VehicleID: eqpt.VEHICLE_ID,
                    //               CST_ID_L: eqpt.CST_ID_L);
                    //        }
                    //        else
                    //        {
                    //            LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //               Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, 沒有CST 在車上。",
                    //               VehicleID: eqpt.VEHICLE_ID,
                    //               CST_ID_L: eqpt.CST_ID_L);
                    //            var remove_result = scApp.TransferService.ForceRemoveCarrierInVehicleByAGV(eqpt.VEHICLE_ID, AGVLocation.Left, "");
                    //            LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //               Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, " +
                    //                     $"沒有CST 在車上,force remove cst ,is success:{remove_result.isSuccess} result:{remove_result.result}。",
                    //               VehicleID: eqpt.VEHICLE_ID,
                    //               CST_ID_L: eqpt.CST_ID_L);
                    //            if (tran.isLoading)
                    //            {
                    //                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //                   Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, " +
                    //                         $"目前命令為Loading:{tran.isLoading},因此將貨的位置更新到Source Port:{SCUtility.Trim(cmd.SOURCE_PORT, true)}。",
                    //                   VehicleID: eqpt.VEHICLE_ID,
                    //                   CST_ID_L: eqpt.CST_ID_L);
                    //                scApp.CarrierBLL.db.updateLocationAndState(cmd.CARRIER_ID, cmd.SOURCE_PORT, E_CARRIER_STATE.WaitIn);
                    //            }
                    //            else if (tran.isUnloading)
                    //            {
                    //                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //                   Data: $"vh:{eqpt.VEHICLE_ID} has mcs cmd:{cmd.TRANSFER_ID} excuting when initial, " +
                    //                         $"目前命令為Unloading:{tran.isLoading},因此將貨的位置更新到Dest Port:{SCUtility.Trim(cmd.DESTINATION_PORT, true)}。",
                    //                   VehicleID: eqpt.VEHICLE_ID,
                    //                   CST_ID_L: eqpt.CST_ID_L);
                    ...
                    //    service.Command.Finish(cmd.ID, completeStatus);
                    //}
                    //else
                    //{
                    //    LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                    //       Data: $"vh:{eqpt.VEHICLE_ID} 無執行命令 when initial。",
                    //       VehicleID: eqpt.VEHICLE_ID,
                    //       CST_ID_L: eqpt.CST_ID_L);
                    //    if (has_cst_on_vh)
                    //    {
                    ...
                    //        if (is_bcr_read_fail)
                    //        {
                    //            var check_carrier_result = scApp.CarrierBLL.db.hasCarrierOnVhLocation(location_id);
                    //            if (check_carrier_result.has)
                    //            {
                    //                new_carrier_id = check_carrier_result.onVhCarrier.ID;
                    ...
                    //            }
                    //        }
                    //        scApp.TransferService.tryInstallCarrierInVehicle(eqpt.VEHICLE_ID, location_id, final_cst_id);
                    //    }
                    //    else
                    //    {
                    //        var remove_result = scApp.TransferService.ForceRemoveCarrierInVehicleByAGV(eqpt.VEHICLE_ID, location_id, "");
                    //    }
                    //    replyTranEventReport(bcfApp, eventType, eqpt, seq_num, "",
                    //        renameCarrierID: final_cst_id);
                    //}
                ...
                if (isTranCmd)
                {
                    ...
                    using (TransactionScope tx = SCUtility.getTransactionScope())
                    {
                        using (DBConnection_EF con = DBConnection_EF.GetUContext())
                        {
                            ...
                            bool isFromVirtualEQPort = false;
                            VTRANSFER vtransfer = scApp.TransferBLL.db.vTransfer.GetVTransferByTransferID(cmdID);
                            if (vtransfer != null)
                            {
                                if (SCUtility.isMatche(vtransfer.HOSTSOURCE, scApp.HPREmptyCarrierVirtualPort))
                                {
                                    isFromVirtualEQPort = true;
                                }
                            }
                            ...
                            {
                                isCreatReportInfoSuccess = scApp.ReportBLL.newReportUnloadArrivals(cmd.TRANSFER_ID, reportqueues, true);
                            }
            ...
            public void GuideInfoRequest(BCFApplication bcfApp, AVEHICLE vh, ID_138_GUIDE_INFO_REQUEST recive_str, int seq_num)
            {
                ...
                try
                {
                    ...
                    //當車子再來重新詢問路徑時，會將他的Reserve 重新清空一次，
                    //避免有殘留的
                    //scApp.ReserveBLL.RemoveAllReservedSectionsByVehicleID(vh.VEHICLE_ID);
                    ...
                    if (is_success && guide_infos.Count > 0)
                    {
                        ...
                        scApp.VehicleBLL.cache.setWillPassSectionInfo(scApp.ReserveBLL, vh.VEHICLE_ID, shortest_path);
                    }
                }
                catch (Exception ex)
                {
                ...
                {
                    vh.isGuideInfoRequesting = false;
                }
            }
                ...
                CalculationPath(AVEHICLE vh, string fromAdr, string toAdr)
            {
                ...
                if (guide_section_isd != null && guide_section_isd.Count > 0)
                {
                    if (!vh.IsOnAdr)
                    {
                        ...
                        start_section_is_same = SCUtility.isMatche(guide_section_isd[0], start_section);
                        if (!start_section_is_same)
                        {
                            guide_section_isd.Insert(0, start_section);
                            ASECTION new_start_section = scApp.SectionBLL.cache.GetSection(start_section);
                            if (SCUtility.isMatche(guide_address_isd[0], new_start_section.FROM_ADR_ID))
                            {
                                guide_address_isd.Insert(0, new_start_section.TO_ADR_ID);
                            }
                            else
                            {
                                guide_address_isd.Insert(0, new_start_section.FROM_ADR_ID);
                            }
                        }
                    }
                }
            ...
            public void StatusReport(BCFApplication bcfApp, AVEHICLE vh, ID_144_STATUS_CHANGE_REP recive_str, int seq_num)
            {
                ...
                if (reserveStatus != vh.RESERVE_PAUSE)
                {
                    vh.onReservedStopStatusChange(reserveStatus);
                }
        ...
        public class AvoidProcessor
        {
                ...
                if (System.Threading.Interlocked.Exchange(ref syncPoint_NotifyVhAvoid, 1) == 0)
                {
                    try
                    {
                        ...
                        if (check_can_creat_avoid_command.is_can)
                        {
                            ...
                            LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                               Data: $"start search avoid address...",
                               VehicleID: requestVhID);
                            ...
                            {
                                LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                                   Data: $"start search avoid address fail. ",
                                   VehicleID: requestVhID);
                                return;
                            }
                            else
                ...
                (AVEHICLE willPassVh, AVEHICLE findAvoidAdrOfVh, bool isDeadLock)
            {
                ...
                ASECTION find_avoid_vh_current_section = null;
                ...
                {
                    find_avoid_vh_current_section = scApp.SectionBLL.cache.GetSection(findAvoidAdrOfVh.CUR_SEC_ID);
                    first_search_adr = findTheOppositeOfAddress(will_pass_vh_cur_adr, find_avoid_vh_current_section);
                }
                ...
                //如果他是在點上的話，也沒必要再往另一個方向找了
                if (!isDeadLock && !searchResult.isFind)
                {
                    string second_search_adr = "";
                    if (findAvoidAdrOfVh.IsOnAdr)
                    {
                        second_search_adr = find_avoid_vh_cur_adr;
                    }
                    else
                    {
                        second_search_adr = SCUtility.isMatche(first_search_adr, find_avoid_vh_current_section.FROM_ADR_ID) ?
                    ...
                    searchResult = tryFindAvoidAddressByOneWay(willPassVh, findAvoidAdrOfVh, second_search_section_infos, true);
                }
                return searchResult;
            }
                ...
                (AVEHICLE willPassVh, AVEHICLE findAvoidAdrVh, (string next_address, ASECTION source_section) startSearchInfo, bool isForceCrossing)
            {
                ...
                ASECTION vh_first_section = startSearchInfo.source_section;
                ...
                try
                {
                    ...
                    do
                    {
                        foreach (var search_info in next_search_infos.ToArray())
                        {
                            ...
                            //先把自己的Section移除
                            if (search_info.source_section != vh_first_section)
                            ...
                            if (next_sections.Count >= 2 &&
                                !SCUtility.isEmpty(search_info.source_section) &&
                            ...
                            foreach (ASECTION sec in next_sections)
                            {
                                ...
                                var block_control_check_result = scApp.getCommObjCacheManager().IsBlockControlSection(sec.SEC_ID);
                                if (block_control_check_result.isBlockControlSec)
                                {
                                    next_search_address_temp.Add((orther_end_point, sec));
                                    LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                                       Data: $"sec id:{SCUtility.Trim(sec.SEC_ID)} is block control section .by pass it,continue find next address{orther_end_point}",
                                       VehicleID: willPassVh.VEHICLE_ID);
                                    continue;
                                }
                                ...
                                if (!reserve_check_result.OK)// 20210623 Mark 改為要看虛擬車是否會阻擋預約
                                {
                                    LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                                       Data: $"sec id:{SCUtility.Trim(sec.SEC_ID)} try to reserve fail,result:{reserve_check_result.Description}.",
                                       VehicleID: willPassVh.VEHICLE_ID);
                                    //如果是虛擬車就繼續往下找
                                    if (reserve_check_result.VehicleID.StartsWith(VehicleVirtualSymbol))
                                    {
                                        next_search_address_temp.Add((orther_end_point, sec));
                                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Debug, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                                           Data: $"sec id:{SCUtility.Trim(sec.SEC_ID)} is reserve fail .by pass it,continue find next address:{orther_end_point}",
                                           VehicleID: willPassVh.VEHICLE_ID);
                                        continue;
                                    }
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (obstruct_vh != null && !obstruct_vh.IsOnAdr && !SCUtility.isMatche(sec.SEC_ID, obstruct_vh.CUR_SEC_ID))
                                            {
                                                next_search_address_temp.Add((orther_end_point, sec));
                                            }
                                        }
        ...
        private void Vh_ReservedStopStatusChange(object sender, VhStopSingle e)
        {
            AVEHICLE vh = sender as AVEHICLE;
            try
            {
                if (e == VhStopSingle.On)
                {
                    vh.isProcessingResetReservedByReservedStopOn = true;
                    scApp.ReserveBLL.RemoveAllReservedSectionsByVehicleID(vh.VEHICLE_ID);
                }
            }
            catch (Exception ex)
            ...
            {
                vh.isProcessingResetReservedByReservedStopOn = false;
            }
        }
        ...
        private void Vh_LocationChange(object sender, LocationChangeEventArgs e)
        {
            ...
            {
                //車子有時在剛進入一段Section時，可能還會跳回到Address上(Section上報空白)
                //因此可能會誤放掉該段Section，故要過濾掉這種情境。
                if (vh.IsOnAdr && leave_section.NodeAddress.Contains(vh.CUR_ADR_ID))
                {
                    //不進行 reserve 釋放
                }
                else
        ...
        public (bool isSuccess, string result) Install(string vhID)
        {
            try
            {
                ...
                if (vh_vo.IsOnAdr)
                {
                    AADDRESS current_adr_obj = scApp.AddressesBLL.cache.GetAddress(vh_vo.CUR_ADR_ID);
                    if (current_adr_obj == null)
                    {
                        string message = $"vh:{vhID} current addiree:{SCUtility.Trim(vh_vo.CUR_ADR_ID, true)} is not exist, can't excute action:Install";
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                           Data: message,
                           VehicleID: vhID);
                        return (false, message);
        ...
        public (bool isSuccess, string result) Remove(string vhID, bool reportmcs)
        {
            try
            {
        ...
        #region Park
        public void askVhToParkForWait(AVEHICLE vh)
        {
            LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleService), Device: DEVICE_NAME_AGV,
                     Data: $"ask vh:{vh.VEHICLE_ID} to park idle.",
                     VehicleID: vh.VEHICLE_ID);
            askVhToPark(vh);
        }

        private (bool isSuccess, string msg) askVhToPark(AVEHICLE vh)
        {
            string vh_current_address = vh.CUR_ADR_ID;
            AADDRESS current_adr = scApp.AddressesBLL.cache.GetAddress(vh.CUR_ADR_ID);
            if (current_adr != null &&
                current_adr is CouplerAddress && (current_adr as CouplerAddress).IsWork(scApp.UnitBLL))
            {
                ...
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(VehicleChargerModule), Device: DEVICE_NAME_AGV,
                         Data: meg,
                         VehicleID: vh.VEHICLE_ID);
                return (false, meg);
            }
