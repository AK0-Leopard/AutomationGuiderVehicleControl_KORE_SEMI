...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    public class CMDDao
    {
        ...
        public ACMD getSourcePortExcuteCMD(DBConnection_EF con, string sourcePort)
        {
            var query = from cmd in con.ACMD
        ...
        public int getExcuteCMDConutLoadPortOrUnloadPort(DBConnection_EF con, string port)
        {
            var query = from cmd in con.ACMD
                        where (cmd.SOURCE_PORT == port.Trim() || cmd.DESTINATION_PORT == port.Trim()) &&
                              cmd.CMD_STATUS < E_CMD_STATUS.NormalEnd
                        select cmd;
            return query.Count();
        }

        public int getExcuteCMDConutByAddresses(DBConnection_EF con, List<string> addresses)
        {
            var query = from cmd in con.ACMD
