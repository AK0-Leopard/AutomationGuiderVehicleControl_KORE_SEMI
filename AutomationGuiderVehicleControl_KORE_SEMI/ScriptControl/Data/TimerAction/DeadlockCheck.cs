...
namespace com.mirle.ibg3k0.sc.Data.TimerAction
{
    ...
    class DeadlockCheck : ITimerAction
    {
        ...
        private int SortOverrideOfVehicleNew(AVEHICLE vh1, AVEHICLE vh2)
        {
            int result;
            if (vh1.CurrentAvoidCount > vh2.CurrentAvoidCount)
            {
                result = -1;
            }
            else if (vh1.CurrentAvoidCount < vh2.CurrentAvoidCount)
            {
                result = 1;
            }
            else
        ...
        public override void doProcess(object obj)
        {
            ...
            if (System.Threading.Interlocked.Exchange(ref checkSyncPoint, 1) == 0)
            {
                try
                {
                    ...
                    vhs_ReserveStop.Sort(SortOverrideOfVehicleNew);
                    ...
                    if (vhs_ReserveStop.Count >= 2)
                    {
                        foreach (var avoid_vh in vhs_ReserveStop)
                        {
                            ...
                            foreach (var wait_pass_vh in wait_pass_vhs)
                            {
                                ...
                                if (avoid_vh.CurrentAvoidCount > wait_pass_vh.CurrentAvoidCount)
                                {
                                    LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(DeadlockCheck), Device: "AGVC",
                                       Data: $"dead lock happend ,但vh:{avoid_vh.VEHICLE_ID} 的避車次數大於 wait_pass_vh:{wait_pass_vh.VEHICLE_ID} 故跳過改下一台車來避車",
                                       VehicleID: avoid_vh.VEHICLE_ID);
                                    break;
                                }
