...
namespace com.mirle.ibg3k0.sc.Data.ValueDefMapAction.AT_S
{
    public class MCSDefaultMapAction : IBSEMDriver, IValueDefMapAction
    {
        ...
        private S6F11.RPTINFO.RPTITEM.VIDITEM_350 buildCurrEqPortStatus()
        {
            ...
            {
                EqPortInfos = new S6F11.RPTINFO.RPTITEM.EQPORTINFO[0]
            };
            return item;
        }
        ...
        protected override void S2F41ReceiveHostCommand(object sender, SECSEventArgs e)
        {
            try
            {
                ...
                if (canCancelCmd)
                {
                    scApp.TransferService.AbortOrCancel(cancel_abort_cmd_id, ProtocolFormat.OHTMessage.CancelActionType.CmdCancel);
                }
                if (canAbortCmd)
                {
        ...
        public override bool S6F11SendVehicleArrived(string cmdID, List<AMCSREPORTQUEUE> reportQueues = null, bool forHPRSpecialReport = false)
        {
            try
            {
                if (!isSend()) return true;
        ...
        public override bool S6F11SendVehicleAcquireStarted(string cmdID, List<AMCSREPORTQUEUE> reportQueues = null, bool forHPRSpecialReport = false)
        {
            try
            {
                if (!isSend()) return true;
        ...
        public override bool S6F11SendVehicleDepositStarted(string cmdID, List<AMCSREPORTQUEUE> reportQueues = null, bool forHPRSpecialReport = false)
        {
            try
            {
                if (!isSend()) return true;
        ...
        public override bool S6F11SendVehicleDepositCompleted(string cmdID, List<AMCSREPORTQUEUE> reportQueues = null, bool forHPRSpecialReport = false)
        {
            try
            {
                if (!isSend()) return true;
        ...
        public override bool S6F11SendCarrierRemoved(string cmdID, List<AMCSREPORTQUEUE> reportQueues = null, bool forHPRSpecialReport = false)
        {
            try
            {
                if (!isSend()) return true;
