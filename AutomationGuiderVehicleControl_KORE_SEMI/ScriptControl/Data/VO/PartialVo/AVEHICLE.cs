...
namespace com.mirle.ibg3k0.sc
{
    ...
    public partial class AVEHICLE : BaseEQObject
    {
        ...
        {
            return MODE_STATUS == VHModeStatus.AutoCharging && batterylevel == BatteryLevel.Low;
        }
        ...
        [JsonIgnore]
        public virtual int CurrentAvoidCount { get; set; } = 0;
        ...
        public virtual bool isHPRVehicle { get; set; }
        ...
        public virtual bool HasExcuteTransferCommand
        {
            get
            {
                return !SCUtility.isEmpty(this.TRANSFER_ID_1) ||
                       !SCUtility.isEmpty(this.TRANSFER_ID_2) ||
                       !SCUtility.isEmpty(this.TRANSFER_ID_3) ||
                       !SCUtility.isEmpty(this.TRANSFER_ID_4);
            }
        }
        [JsonIgnore]
        ...
        [JsonIgnore]
        public bool isGuideInfoRequesting = false;
        [JsonIgnore]
        public bool isProcessingResetReservedByReservedStopOn = false;
        ...
        public class GuideData
        {
            ...
            public List<Section> GuideSections { get; private set; }
            public List<string> GuideAddress { get; private set; }
            public GuideData(sc.BLL.ReserveBLL reserveBLL, List<string> giudeSectionIDs, List<string> guideAddresses)
            {
                GuideSections = new List<Section>();
                GuideAddress = guideAddresses.ToList();
            ...
            public void put(GuideData guideData)
            {
                GuideSections.Clear();
                GuideAddress.Clear();
                GuideSections.AddRange(guideData.GuideSections);
                GuideAddress.AddRange(guideData.GuideAddress);
            }
            public void clear()
            {
                GuideSections.Clear();
            ...
            public void refreshPassStatus(string leaveSection)
            {
                try
                {
                    if (GuideSections == null) return;
                    var sec_obj = GuideSections.Where(s => SCUtility.isMatche(s.ID, leaveSection)).FirstOrDefault();
                    if (sec_obj == null) return;
                    int sec_index = GuideSections.IndexOf(sec_obj);
                    for (int i = 0; i <= sec_index; i++)
                    {
                        GuideSections[i].setIsPassFlag();
                    }
                }
                catch (Exception ex)
                {
            ...
            {
                var sec_obj = GuideSections.Where(s => SCUtility.isMatche(s.ID, secID)).FirstOrDefault();
                if (sec_obj == null)
                {
            ...
            public (bool isExist, List<string> sectionIDs) tryGetGuideSectionIDs()
            {
                ...
                return (true, GuideSections.Select(s => s.ID).ToList());
            }
